...
- (void)registerAuthenticationCertificate:(CDVInvokedUrlCommand*)command
{
    ...
    NSArray *p12Files = [dirFiles filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"self ENDSWITH '.p12'"]];
    NSString *p12_name = [p12Files objectAtIndex:0];
    NSString *full_path = [path stringByAppendingPathComponent:(p12_name)];
    ...
    SecTrustRef myTrust;
    OSStatus status = extractIdentityAndTrust(full_path, password, &myIdentity, &myTrust);
    if(status != noErr) {
    ...
    NSString* networkID = copySummaryString(myIdentity);
    
//    CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
    CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:networkID];
    [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
}
